...
const checkReact = async (): Promise<boolean> => {
  ...
  try {
    ...
    const dependenciesCheck: boolean = dependencies.hasOwnProperty('react') && dependencies.hasOwnProperty('react-dom');
    const devDependenciesCheck: boolean =
      devDependencies.hasOwnProperty('react') && devDependencies.hasOwnProperty('react-dom');
