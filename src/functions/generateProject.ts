...
import { convertToTitleCase, sleep, spinner } from '../utilities/utility.js';
...
const generateProjects = async (program: Command) => {
  ...
  if (isReact) {
    ...
    if (program.args[1] === MaxCommand.COMPONENT || program.args[1] === MinCommand.COMPONENT) {
      ...
      if (!program.args[2]) {
        spinner.error({
          text: `${chalk.red('Filename is required.')}Correct usage is ${chalk.green(
            'npx arclix generate component <filename>'
          )}\n`,
        });
        return;
      }
      ...
      const obj = GenerateProject.getInstance(
        {
      ...
      if (options.flat) {
        obj.generateComponent(!options.skipTest); // !options.skipTest -> includeTest
      } else {
      ...
      if (fileCreationError) {
        ...
        spinner.success({
          text: `Component ${convertToTitleCase(chalk.green(program.args[2]))} created\n`,
        });
      }
    } else {
